...
func resourceWAFCreate(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        return fmt.Errorf("Error creating twcc_waf %s on %s: %v", name, platform, err)
    }
    ...
    if err != nil {
        return fmt.Errorf(
            "Error waiting for twcc_waf %d to become Ready: %v", siteID, err)
    }
...
func resourceWAFRead(d *schema.ResourceData, meta interface{}) error {
    ...
    log.Printf("[DEBUG] Retrieved twcc_waf %s", d.Id())
...
func resourceWAFDelete(d *schema.ResourceData, meta interface{}) error {
    ...
    if err != nil {
        return fmt.Errorf( 
            "Error waiting for twcc_waf %s to become Deleted: %v", siteID, err)
    }
